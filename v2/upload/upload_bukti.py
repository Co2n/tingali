...
for fid in target_fids:
    ...
    response = supabase.table(nama_tabel).select("fid, bukti").eq("fid", fid).execute()
    ...
    local_path = baris_data['bukti']
    
    # Lewati jika path kosong di database
    if not local_path:
        print(f"LEWATI: fid {fid} kolom bukti-nya kosong.")
        continue
    ...
    # folder_utama = r"C:\Data_QGIS\Folder_bukti"
    ...
    if os.path.exists(local_path):
        print(f"Proses bukti untuk fid: {fid}...")
        try:
            ...
            new_file_name = f"bukti_{fid}_{datenow}{ext}"
            ...
            supabase.table(nama_tabel).update({"bukti": public_url}).eq("fid", fid).execute()
