...
for fid in target_fids:
    ...
    response = supabase.table(nama_tabel).select("fid, kk").eq("fid", fid).execute()
    ...
    local_path = baris_data['kk']
    
    # Lewati jika path kosong di database
    if not local_path:
        print(f"LEWATI: fid {fid} kolom kk-nya kosong.")
        continue
    ...
    # folder_utama = r"C:\Data_QGIS\Folder_kk"
    ...
    if os.path.exists(local_path):
        print(f"Proses kk untuk fid: {fid}...")
        try:
            ...
            new_file_name = f"kk_{fid}_{datenow}{ext}"
            ...
            supabase.table(nama_tabel).update({"kk": public_url}).eq("fid", fid).execute()
