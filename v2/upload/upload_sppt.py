...
for fid in target_fids:
    ...
    response = supabase.table(nama_tabel).select("fid, sppt").eq("fid", fid).execute()
    ...
    local_path = baris_data['sppt']
    
    # Lewati jika path kosong di database
    if not local_path:
        print(f"LEWATI: fid {fid} kolom sppt-nya kosong.")
        continue
    ...
    # folder_utama = r"C:\Data_QGIS\Folder_sppt"
    ...
    if os.path.exists(local_path):
        print(f"Proses sppt untuk fid: {fid}...")
        try:
            ...
            new_file_name = f"sppt_{fid}_{datenow}{ext}"
            ...
            supabase.table(nama_tabel).update({"sppt": public_url}).eq("fid", fid).execute()
