...
for fid in target_fids:
    ...
    response = supabase.table(nama_tabel).select("fid, ktp").eq("fid", fid).execute()
    ...
    local_path = baris_data['ktp']
    
    # Lewati jika path kosong di database
    if not local_path:
        print(f"LEWATI: fid {fid} kolom ktp-nya kosong.")
        continue
    ...
    # folder_utama = r"C:\Data_QGIS\Folder_KTP"
    ...
    if os.path.exists(local_path):
        print(f"Proses KTP untuk fid: {fid}...")
        try:
            ...
            new_file_name = f"ktp_{fid}_{datenow}{ext}"
            ...
            supabase.table(nama_tabel).update({"ktp": public_url}).eq("fid", fid).execute()
