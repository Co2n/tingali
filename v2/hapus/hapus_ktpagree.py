...
# cara jalaninnya > python hapus_ktpagree.py
import csv
import urllib.parse
...
key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ0bHpxcndybGl2Z3N6b213aGhiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MzA0NzgwMywiZXhwIjoyMDg4NjIzODAzfQ._cpBfS42rnPzdI0eRQfYU2KzCG4h2uepEv095gIyS7E"
...
for fid in target_fids:
    ...
    response = supabase.table(nama_tabel).select("ktpagree").eq("fid", fid).execute()
    ...
    url_ktpagree = response.data[0]['ktpagree']
    
    # Cek apakah kolom ktpagree sudah kosong
    if not url_ktpagree:
        print(f"  -> LEWATI: Kolom ktpagree untuk fid {fid} sudah kosong.")
        continue

    try:
        # Ekstrak nama file dari URL
        file_name = url_ktpagree.split('/')[-1]
        ...
        # Kosongkan kolom 'ktpagree' di tabel
        supabase.table(nama_tabel).update({"ktpagree": None}).eq("fid", fid).execute()
